...
import {
  ...
  Sparkles, TrendingUp, BookOpen, AlertTriangle, Upload, XCircle,
} from 'lucide-react';
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    if (!profile) return;
    Promise.all([
      supabase.from('agent_applications').select('*, courses(title)').eq('student_id', profile.id).order('created_at', { ascending: false }).limit(3),
      supabase.from('forms').select('*').eq('student_id', profile.id).order('created_at', { ascending: false }).limit(3),
      supabase.from('messages').select('*').eq('student_id', profile.id).eq('is_read', false).order('created_at', { ascending: false }).limit(5),
      supabase.from('documents').select('*').eq('student_id', profile.id).order('created_at', { ascending: false }),
    ]).then(async ([apps, frms, msgs, docsData]) => {
      setApplications(apps.data ?? []);
      setForms(frms.data ?? []);
      setMessages(msgs.data ?? []);
      setDocs(docsData.data ?? []);

      // Get document requests for student's applications
      const appIds = (apps.data ?? []).map((a: any) => a.id);
      if (appIds.length > 0) {
        const { data: reqs } = await supabase.from('document_requests').select('*').in('application_id', appIds).eq('status', 'pending').order('created_at', { ascending: false });
        setDocRequests(reqs ?? []);
      }
  ...
  const quickLinks = [
    ...
    { icon: FolderOpen, label: 'Upload Docs', href: '/student/documents', gradient: 'from-sky-500 to-blue-700' },
    { icon: MessageSquare, label: 'Messages', href: '/student/messages', gradient: 'from-blue-700 to-blue-800', badge: messages.length },
    { icon: Calendar, label: 'Applications', href: '/student/applications', gradient: 'from-blue-400 to-blue-700' },
  ];
  ...
  return (
    ...
    >
        ...
        <div className="relative z-10 p-5 sm:p-6 md:p-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              >
                <BookOpen className="w-4 h-4" /> {applications.length === 0 ? 'Apply Now' : 'New Application'}
              </Link>
            </div>
          </div>
      ...
      {(docRequests.length > 0 || applications.some(a => a.status === 'documents_required')) && (
        <motion.div variants={fadeUp} className="rounded-2xl border-2 border-red-200 bg-red-50 p-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-red-700 text-xs mt-1">
                {docRequests.length > 0
                  ? `You have ${docRequests.length} document request${docRequests.length > 1 ? 's' : ''} pending. Please upload the requested documents.`
                  : 'One or more applications require additional documents.'}
              </p>
              <Link to="/student/documents" className="inline-flex items-center gap-1 mt-2 text-xs font-semibold text-red-700 hover:text-red-900">
                <Upload className="w-3.5 h-3.5" /> Upload Documents <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
        </motion.div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={fadeUp} className="card p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-slate-900 flex items-center gap-2">
              <FileText className="w-4 h-4 text-blue-700" /> My Applications
            </h2>
            <Link to="/student/applications" className="text-sm text-blue-700 hover:text-blue-700 font-semibold flex items-center gap-1">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {applications.map((app) => {
                const isIncomplete = app.status === 'documents_required' || app.status === 'rejected';
                return (
                  <Link
                    key={app.id}
                    to={`/student/applications/${app.id}`}
                    className={`flex items-center gap-3 p-3 rounded-xl transition-colors cursor-pointer border-2 ${
                      isIncomplete
                  ...
                  >
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      isIncomplete ? 'bg-red-100' : 'bg-gradient-to-br from-blue-500 to-blue-700'
                    }`}>
                      {isIncomplete ? <AlertCircle className="w-4 h-4 text-red-500" /> : <FileText className="w-4 h-4 text-white" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {app.status === 'documents_required' && (
                        <p className="text-xs text-red-600 font-medium mt-0.5">Documents needed</p>
                      )}
                      {app.status === 'rejected' && app.public_comment && (
                        <p className="text-xs text-red-600 truncate mt-0.5">{app.public_comment}</p>
                      )}
                    </div>
        ...
        <motion.div variants={fadeUp} className="card p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-bold text-slate-900 flex items-center gap-2">
              <FolderOpen className="w-4 h-4 text-blue-700" /> Recent Forms
            </h2>
            <Link to="/student/forms" className="text-sm text-blue-700 hover:text-blue-700 font-semibold flex items-center gap-1">
              View all <ArrowRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          {loading ? (
      ...
      <motion.div variants={fadeUp} className="card p-6">
        <h2 className="font-bold text-slate-900 mb-5 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-blue-700" /> Important Dates
        </h2>
