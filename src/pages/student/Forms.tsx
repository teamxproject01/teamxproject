      ...
      >
        ...
        <div className="px-5 pt-4">
          <div className="flex gap-2">
            {steps.map((s, i) => (
              <div key={s} className={`flex-1 h-1.5 rounded-full transition-all ${i <= step ? 'bg-blue-700' : 'bg-slate-200'}`} />
            ))}
          </div>
        </div>
                ...
                <div className="space-y-2">
                  {REASONS.map((r) => (
                    <label key={r} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/40 cursor-pointer transition-all">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${data.reason === r ? 'border-blue-700 bg-blue-700' : 'border-slate-300'}`}>
                ...
                <div className="space-y-2">
                  {OUTCOMES.map((o) => (
                    <label key={o} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:border-blue-300 hover:bg-blue-50/40 cursor-pointer transition-all">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${data.requestedOutcome === o ? 'border-blue-700 bg-blue-700' : 'border-slate-300'}`}>
          ...
          {step === 3 && (
            <>
              ...
              <label className="flex items-start gap-3 cursor-pointer p-4 border-2 rounded-xl border-slate-200 hover:border-blue-300 transition-colors">
                <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-all ${data.declaration ? 'bg-blue-700 border-blue-700' : 'border-slate-300'}`}>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {FORM_TYPES.map(({ type, desc }) => (
                ...
                >
                  <div className="w-9 h-9 bg-blue-100 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:bg-blue-700 transition-colors">
                    <FileText className="w-4 h-4 text-blue-700 group-hover:text-white transition-colors" />
                  </div>
                  <div>
