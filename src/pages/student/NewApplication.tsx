...
export default function StudentNewApplication() {
  ...
  const updateScore = (field: keyof EnglishScoreFields, value: string) =>
    setEnglishScores(prev => ({ ...prev, [field]: value }));
  ...
  const handleSubmit = async () => {
    if (!profile) return;
    if (!form.student_full_name || !form.student_email || !form.course_id || !form.preferred_intake) {
    ...
    const { data, error: insertError } = await supabase.from('agent_applications').insert({
      ...
      student_nationality: form.student_nationality || null,
      student_current_country: form.student_current_country || null,
      student_passport_number: form.student_passport_number || null,
      student_education_level: form.student_education_level || null,
      student_english_test_status: form.student_english_test_status || null,
      preferred_intake: form.preferred_intake,
      agent_notes: form.notes || null,
  ...
  const steps = [
    { num: 1, label: 'My Info', icon: User },
    { num: 2, label: 'Course & Intake', icon: BookOpen },
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 1 && (
          <motion.div key="step1" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-4">
            <h2 className="font-bold text-slate-900 mb-2">Your Information</h2>
        ...
        {step === 2 && (
          <motion.div key="step2" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-4">
            <h2 className="font-bold text-slate-900 mb-2">Select Course & Intake</h2>
            <div>
              <label className="label">Preferred Intake *</label>
              <select value={form.preferred_intake} onChange={(e) => update('preferred_intake', e.target.value)} className="input-field">
        ...
        {step === 4 && (
          <motion.div key="step4" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-4">
            ...
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-bold text-slate-700 mb-2">Your Information</h3>
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-700 mb-2">Selected Course & Intake</h3>
                <div className="bg-slate-50 p-4 rounded-xl space-y-1">
                  ...
                  <div className="flex items-center gap-2 mt-2 text-sm">
                    ...
                    <span className="font-medium text-slate-800">{form.preferred_intake || '—'}</span>
                  </div>
                </div>
              </div>
