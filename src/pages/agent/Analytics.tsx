...
export default function Analytics() {
  ...
  const analytics = useMemo(() => {
    ...
    const offerReceived = apps.filter(a => ['offer_letter_sent', 'waiting_for_signature', 'signed_offer_review', 'offer_accepted'].includes(a.status)).length;
    ...
    const funnel = [
      ...
      { stage: 'Under Review', count: apps.filter(a => ['under_review', 'documents_required'].includes(a.status)).length },
  ...
  const kpiCards = [
    { label: 'Monthly Applications', value: analytics.monthly, icon: TrendingUp, color: 'bg-blue-700' },
    ...
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      ...
      <motion.div variants={fadeUp} className="card p-6">
        <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
          <BarChart3 className="w-5 h-5 text-blue-700" /> Monthly Applications
        </h3>
      ...
      <motion.div variants={fadeUp} className="card p-6">
        <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
          <TrendingUp className="w-5 h-5 text-blue-700" /> Application Conversion Funnel
        </h3>
        <ResponsiveContainer width="100%" height={250}>
