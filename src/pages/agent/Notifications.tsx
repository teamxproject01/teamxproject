  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        setNotifs(data ?? []);
        setLoading(false);
        const unread = (data ?? []).filter((n: AgentNotification) => !n.is_read);
        if (unread.length > 0) {
          supabase.from('agent_notifications').update({ is_read: true }).eq('user_id', profile.id).eq('is_read', false)
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between">
        ...
        {unreadCount > 0 && (
          <button onClick={markAllRead} className="text-sm text-blue-700 font-semibold hover:text-blue-700">
            Mark all as read
          </button>
        )}
      </div>
                ...
                <motion.div key={n.id} variants={fadeUp} initial="hidden" animate="visible">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${typeColors[n.type] ?? 'bg-blue-100 text-blue-700'}`}>
            ...
            {selected ? (
              <div className="card p-6 lg:sticky lg:top-4">
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${typeColors[selected.type] ?? 'bg-blue-100 text-blue-700'}`}>
