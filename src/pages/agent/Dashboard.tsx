...
const COLORS = ['#2563eb', '#60a5fa', '#10B981', '#F59E0B', '#EF4444', '#1e3a8a', '#06B6D4', '#84CC16', '#F97316', '#3b82f6', '#94A3B8'];
...
export default function AgentDashboard() {
  ...
  const pending = apps.filter(a => ['submitted', 'under_review', 'documents_required'].includes(a.status)).length;
  const approved = apps.filter(a => ['approved', 'offer_letter_sent', 'waiting_for_signature', 'signed_offer_review', 'offer_accepted', 'waiting_for_payment', 'paid_coe_issued', 'enrolled'].includes(a.status)).length;
  ...
  const statCards = [
    { label: 'Total Applications', value: total, icon: FileText, color: 'bg-blue-700' },
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="space-y-6">
      {/* Welcome banner */}
      <motion.div variants={fadeUp} className="bg-gradient-to-r from-blue-700 to-sky-500 rounded-2xl p-6 text-white">
      ...
      <motion.div variants={fadeUp}>
        <Link to="/agent/new-application" className="card p-5 flex items-center gap-4 hover:shadow-card-hover transition-all group">
          <div className="w-12 h-12 bg-blue-700 rounded-xl flex items-center justify-center group-hover:scale-105 transition-transform">
          ...
          <div className="text-blue-700 font-semibold text-sm group-hover:translate-x-1 transition-transform">Start →</div>
        </Link>
      </motion.div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <motion.div variants={fadeUp} className="card p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/agent/applications" className="text-sm text-blue-700 font-semibold hover:text-blue-700">View all</Link>
          </div>
          {recentApps.length === 0 ? (
        ...
        <motion.div variants={fadeUp} className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/agent/notifications" className="text-sm text-blue-700 font-semibold hover:text-blue-700">All</Link>
          </div>
          {notifs.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-32 text-slate-400">
