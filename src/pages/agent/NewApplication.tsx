...
import {
  ...
  FileText, X, ShieldCheck, Heart, Briefcase, Plane, FileSignature, HelpCircle,
} from 'lucide-react';
...
export default function NewApplication() {
  ...
  const stepValid = (() => {
    ...
    if (step === 6) return !validateGSA(gsaData, true);
    return true;
  })();
  ...
  const buildPayload = (status: 'draft' | 'submitted') => {
    ...
    o.gsa_agent_decl_1 = gsaData.gsa_agent_decl_1;
    o.gsa_agent_decl_2 = gsaData.gsa_agent_decl_2;
    o.gsa_agent_decl_3 = gsaData.gsa_agent_decl_3;
    o.gsa_agent_decl_name = gsaData.gsa_agent_decl_name || null;
    o.gsa_agent_decl_date = gsaData.gsa_agent_decl_date || null;
    o.gsa_agent_decl_signature = gsaData.gsa_agent_decl_signature || null;
    if (status === 'submitted') {
      o.source = 'agent';
  ...
  const handleSaveDraft = async () => {
    ...
    if (isDraft && editId) {
      ...
      const { data: inserted, error: insertError } = await supabase.from('agent_applications')
        .insert({ agent_id: profile.id, ...buildPayload('draft') }).select().single();
  ...
  const handleSubmit = async () => {
    if (!profile) return;
    if (!form.student_first_name || !form.student_last_name || !form.student_email || !form.course_id) {
    ...
    if (isDraft && editId) {
      ...
      const { data: inserted, error: insertError } = await supabase.from('agent_applications')
        .insert({ agent_id: profile.id, ...buildPayload('submitted') }).select().single();
      if (insertError) { setError(insertError.message); setLoading(false); return; }
      data = inserted;
    }
    if (data) {
    ...
    for (const doc of documents) {
      ...
      if (existing && existing.length > 0) {
        for (const ex of existing) {
          if (ex.file_path) await supabase.storage.from('agent-documents').remove([ex.file_path]);
          await supabase.from('application_documents').delete().eq('id', ex.id);
        }
      }
      const filePath = `${profile.id}/${appId}/${Date.now()}_${doc.file.name}`;
      const { error: uploadError } = await supabase.storage.from('agent-documents').upload(filePath, doc.file);
      if (!uploadError) {
        await supabase.from('application_documents').insert({
          application_id: appId, agent_id: profile.id, document_type: doc.type,
          file_path: filePath, file_name: doc.file.name, file_size: doc.file.size, status: 'uploaded',
        });
      }
    }
  };
  ...
  const steps = [
    ...
    { num: 7, label: 'Review', icon: CheckCircle },
  ];
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">{isDraft ? 'Edit Draft Application' : 'New Student Application'}</h1>
        <p className="text-slate-500 text-sm mt-1">{isDraft ? 'Update and submit your saved draft' : 'Create a new application for a prospective student'}</p>
      </div>
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="step1" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-6">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <div>
                ...
                <select value={form.student_title} onChange={(e) => update('student_title', e.target.value)} className={inputCls}>
              ...
              <div>
                ...
                <input type="text" value={form.student_first_name} onChange={(e) => update('student_first_name', e.target.value)} className={inputCls} placeholder="John" />
              </div>
              <div>
                <label className={labelCls}>Last Name *</label>
                <input type="text" value={form.student_last_name} onChange={(e) => update('student_last_name', e.target.value)} className={inputCls} placeholder="Smith" />
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <select value={form.student_gender} onChange={(e) => update('student_gender', e.target.value)} className={inputCls}>
                  <option value="">Select…</option>
              ...
              <div>
                ...
                <input type="date" value={form.student_dob} onChange={(e) => update('student_dob', e.target.value)} className={inputCls} />
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" value={form.student_country_of_birth} onChange={(e) => update('student_country_of_birth', e.target.value)} className={inputCls} placeholder="Australia" />
              </div>
              <div>
                <label className={labelCls}>Nationality *</label>
                <input type="text" value={form.student_nationality} onChange={(e) => update('student_nationality', e.target.value)} className={inputCls} placeholder="Australian" />
              </div>
              <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={form.student_passport_number} onChange={(e) => update('student_passport_number', e.target.value)} className={inputCls} placeholder="P1234567" />
              </div>
              <div>
                <label className={labelCls}>Passport Expiry *</label>
                <input type="date" value={form.student_passport_expiry} onChange={(e) => update('student_passport_expiry', e.target.value)} className={inputCls} />
              </div>
              <div>
                <label className={labelCls}>Mobile Number *</label>
                <input type="tel" value={form.student_mobile_number} onChange={(e) => update('student_mobile_number', e.target.value)} className={inputCls} placeholder="+61 400 000 000" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="email" value={form.student_email} onChange={(e) => update('student_email', e.target.value)} className={inputCls} placeholder="student@email.com" />
              </div>
            </div>

            <div className="space-y-2">
              <label className={labelCls}>Are you currently in Australia? *</label>
              {yesNo('student_in_australia')}
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div><label className={labelCls}>Street Number *</label><input type="text" value={form.residential_street_number} onChange={(e) => update('residential_street_number', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Street Name *</label><input type="text" value={form.residential_street_name} onChange={(e) => update('residential_street_name', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Suburb/Town *</label><input type="text" value={form.residential_suburb} onChange={(e) => update('residential_suburb', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>City *</label><input type="text" value={form.residential_city} onChange={(e) => update('residential_city', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>State *</label><input type="text" value={form.residential_state} onChange={(e) => update('residential_state', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Country *</label><input type="text" value={form.residential_country} onChange={(e) => update('residential_country', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Post Code *</label><input type="text" value={form.residential_post_code} onChange={(e) => update('residential_post_code', e.target.value)} className={inputCls} /></div>
              </div>
            </div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="space-y-2 mb-3">
                <label className={labelCls}>Is your postal address the same as your residential address? *</label>
                {yesNo('postal_same_as_residential')}
              </div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div><label className={labelCls}>Full Name *</label><input type="text" value={form.emergency_full_name} onChange={(e) => update('emergency_full_name', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Relationship *</label><input type="text" value={form.emergency_relationship} onChange={(e) => update('emergency_relationship', e.target.value)} className={inputCls} placeholder="Parent" /></div>
                <div><label className={labelCls}>Address *</label><input type="text" value={form.emergency_address} onChange={(e) => update('emergency_address', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Mobile *</label><input type="tel" value={form.emergency_mobile} onChange={(e) => update('emergency_mobile', e.target.value)} className={inputCls} /></div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={form.first_language} onChange={(e) => update('first_language', e.target.value)} className={inputCls}>
                    <option value="">Select…</option>
                ...
                <div>
                  <label className={labelCls}>English Test Status *</label>
                  <select value={form.student_english_test_status} onChange={(e) => { update('student_english_test_status', e.target.value); setEnglishScores(emptyEnglishScores); }} className={inputCls}>
                    <option value="">Select…</option>
                    {englishTests.map(t => <option key={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label className={labelCls}>How well do you speak English? *</label>
                  <select value={form.english_speaking_level} onChange={(e) => update('english_speaking_level', e.target.value)} className={inputCls}>
                    <option value="">Select…</option>
        ...
        {step === 2 && (
          <motion.div key="step2" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-4">
            ...
            <div>
              <label className={labelCls}>MIHE Intake (trimester) *</label>
              <select value={form.preferred_intake} onChange={(e) => update('preferred_intake', e.target.value)} className={inputCls}>
                <option value="">— Select —</option>
                {intakes.map(i => <option key={i}>{i}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              <p className="text-sm font-medium text-slate-700">Tick the course you wish to apply for *</p>
              {courses.map(course => (
                <label key={course.id} className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all ${form.course_id === course.id ? 'border-blue-500 bg-blue-50' : 'border-slate-100 hover:border-blue-200 hover:bg-slate-50'}`}>
        ...
        {step === 3 && (
          <motion.div key="step3" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-6">
            ...
            <div className="space-y-2">
              <label className={labelCls}>Do you have any disabilities or medical conditions that you would like MIHE to be aware of? *</label>
              {yesNo('disability_has')}
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className={labelCls}>Highest COMPLETED qualification level *</label>
                  <select value={form.qualification_level} onChange={(e) => update('qualification_level', e.target.value)} className={inputCls}>
                    <option value="">Select…</option>
                    {educationLevels.map(l => <option key={l}>{l}</option>)}
                  </select>
                </div>
                <div><label className={labelCls}>Qualification Name *</label><input type="text" value={form.qualification_name} onChange={(e) => update('qualification_name', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Year of Completion *</label><input type="text" value={form.qualification_year} onChange={(e) => update('qualification_year', e.target.value)} className={inputCls} placeholder="2020" /></div>
                <div><label className={labelCls}>Institution Name *</label><input type="text" value={form.qualification_institution} onChange={(e) => update('qualification_institution', e.target.value)} className={inputCls} /></div>
                <div><label className={labelCls}>Country (where qualification completed) *</label><input type="text" value={form.qualification_country} onChange={(e) => update('qualification_country', e.target.value)} className={inputCls} /></div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="space-y-3">
                <div><label className={labelCls}>Applying for Recognition of Prior Learning? *</label>{yesNo('applying_rpl')}</div>
                <div><label className={labelCls}>Applying for a Credit Transfer? *</label>{yesNo('applying_credit_transfer')}</div>
              </div>
            </div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <label className={labelCls}>Current employment status *</label>
              <select value={form.employment_status} onChange={(e) => update('employment_status', e.target.value)} className={inputCls}>
                <option value="">Select…</option>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="space-y-3">
                <div><label className={labelCls}>Do you hold any visa for Australia? *</label>{yesNo('visa_held')}</div>
                <div><label className={labelCls}>Have you previously held any visa for Australia? *</label>{yesNo('visa_previously_held')}</div>
                <div>
                  <label className={labelCls}>Have you ever applied for a visa for Australia that was granted or rejected? *</label>
                  <select value={form.visa_application_status} onChange={(e) => update('visa_application_status', e.target.value)} className={inputCls}>
                    <option value="">Select…</option>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="space-y-3">
                <div>
                  <label className={labelCls}>Do you meet the criteria of a Genuine Temporary Entrant (GTE)? *</label>
                  {yesNo('gte_meets')}
                ...
                <div><label className={labelCls}>Do you hold sufficient funds for the first year of the applied course fee? *</label>{yesNo('funds_first_year')}</div>
                <div><label className={labelCls}>Do you hold sufficient funds for cost of living in Australia (for an individual AUD $29,710)? *</label>{yesNo('funds_living')}</div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="space-y-3">
                <div><label className={labelCls}>Would you like MIHE to arrange your Overseas Student Health Cover (OSHC) for your entire period of study? *</label>{yesNo('oshc_arrange')}</div>
                <div><label className={labelCls}>Would you like us to arrange an airport pickup at the time of your arrival in Australia? *</label>{yesNo('airport_pickup')}</div>
              </div>
            </div>
            ...
            <div className="border-t border-slate-100 pt-4">
              ...
              <div className="space-y-2">
                <label className={labelCls}>Do you require any additional support while studying at MIHE? *</label>
                {yesNo('additional_support_required')}
        ...
        {/* Step 4: Documents */}
        {step === 4 && (
          <motion.div key="step4docs" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      <label className="flex-1 flex items-center gap-2 p-3 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-blue-400 transition-colors">
                        ...
                        <span className="text-sm text-slate-600 truncate">{doc.file ? doc.file.name : doc.existingName ? `${doc.existingName} (uploaded)` : 'Choose file…'}</span>
                      </label>
                      {documents.length > 1 && (
        ...
        {step === 5 && (
          <motion.div key="step5decl" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-4">
            <div className="flex items-center gap-2">
            ...
            <div className="border-t border-slate-100 pt-4 grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={form.student_decl_name} onChange={(e) => update('student_decl_name', e.target.value)} className={inputCls} placeholder="Full name" />
              </div>
              <div>
                <label className={labelCls}>Signature (type your full name) *</label>
                <input type="text" value={form.student_decl_signature} onChange={(e) => update('student_decl_signature', e.target.value)} className={inputCls} placeholder="Type your full name" style={{ fontFamily: 'Georgia, serif' }} />
              </div>
              <div>
        ...
        {/* Step 6: GSA */}
        {step === 6 && (
          <motion.div key="step4gsa" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="space-y-4">
            ...
            <GSAForm data={gsaData} onChange={setGSAData} isAgentSubmitted={true} autoFill={gsaAutoFill} />
          </motion.div>
        )}

        {/* Step 7: Review */}
        {step === 7 && (
          <motion.div key="step7review" variants={fadeUp} initial="hidden" animate="visible" exit={{ opacity: 0 }} className="card p-6 space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-700 mb-2">Documents ({documents.filter(d => d.file || d.existingName).length} uploaded)</h3>
                <div className="bg-slate-50 p-4 rounded-xl space-y-1">
                  {documents.filter(d => d.file || d.existingName).length === 0 ? (
                    <p className="text-slate-400 text-sm">No documents uploaded</p>
                  ) : documents.filter(d => d.file || d.existingName).map((d, i) => (
                    <div key={i} className="flex items-center gap-2 text-sm">
                      <FileText className="w-4 h-4 text-blue-500" />
                      <span className="text-slate-700">{d.type}:</span>
                      <span className="text-slate-500">{d.file?.name || d.existingName}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <GSASummary app={{ ...gsaData, source: 'agent' }} defaultOpen={true} />
              </div>
