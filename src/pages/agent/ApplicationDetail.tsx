...
export default function ApplicationDetail() {
  ...
  const backPath = isStudent ? '/student/applications' : '/agent/applications';
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>, docType?: string, replaceDocId?: string, requestId?: string) => {
    ...
    await supabase.from('application_documents').insert({
      application_id: id,
      ...(isStudent ? { student_id: profile.id } : { agent_id: profile.id }),
      document_type: type,
      file_path: filePath,
      file_name: file.name,
      file_size: file.size,
  ...
  const sendMessage = async () => {
    ...
    if (!convId) {
      const { data: newConv } = await supabase.from('conversations').insert({
        ...
        student_id: isStudent ? profile.id : (app.student_id || null),
        agent_id: isStudent ? (app.agent_id || null) : profile.id,
    ...
    await supabase.from('conversation_messages').insert({
      ...
      sender_role: isStudent ? 'student' : 'agent',
  ...
  if (!app) {
    return (
      <div className="text-center py-20">
        ...
        <Link to={backPath} className="text-blue-700 font-semibold text-sm mt-2 inline-block">Back to applications</Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center gap-3 sm:gap-4">
        <button onClick={() => navigate(backPath)} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100 transition-colors flex-shrink-0">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Conversation Thread */}
          <CollapsibleCard title="Conversation with Admissions" icon={MessageSquare} badge={messages.length > 0 ? <span className="text-xs text-slate-500 ml-2">{messages.length} messages</span> : undefined}>
            {messages.length > 0 && (
              <div className="space-y-3 mb-4 max-h-64 overflow-y-auto">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex flex-col ${(msg.sender_role === 'agent' && !isStudent) || (msg.sender_role === 'student' && isStudent) ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[80%] p-3 rounded-xl ${(msg.sender_role === 'agent' && !isStudent) || (msg.sender_role === 'student' && isStudent) ? 'bg-blue-700 text-white' : 'bg-slate-100 text-slate-800'}`}>
            ...
            <div className="space-y-2">
              <textarea value={messageText} onChange={(e) => setMessageText(e.target.value)} placeholder="Type a message to admissions…" rows={3} className="input-field text-sm resize-none" />
