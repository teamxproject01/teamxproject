...
export default function BecomeAgent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const err = validateStep();
    if (err) { setError(err); setStep(STEPS.length - 1); return; }
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input required type="text" value={form.fullName} onChange={(e) => set('fullName', e.target.value)} className="input-field pl-10" placeholder="Jane Smith" />
                          </div>
                        </div>
                        <div>
