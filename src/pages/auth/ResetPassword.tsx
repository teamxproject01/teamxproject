...
import { Lock, Eye, EyeOff, AlertCircle, ShieldCheck, CheckCircle } from 'lucide-react';
...
export default function ResetPassword() {
  ...
  const { clearMustResetFlag, user, profile } = useAuth();

  useEffect(() => {
    // Detect recovery mode from the URL hash immediately (Supabase appends type=recovery)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => {
        if (recoveryMode && !profile) {
          supabase.auth.signOut();
          navigate('/login');
        } else if (profile?.role === 'agent') {
          navigate('/agent/dashboard');
        } else if (profile && ['admin', 'super_admin', 'admissions', 'finance', 'marketing', 'support'].includes(profile.role)) {
        ...
        <div className="card p-6 sm:p-8">
          {success ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-slate-500">Your password has been changed successfully. Redirecting you to your dashboard…</p>
            </div>
          ) : (
            <>
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h1 className="text-xl font-bold text-slate-900">
                    {recoveryMode ? 'Reset Your Password' : 'Set Your Password'}
                  </h1>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <Button type="submit" loading={loading} className="w-full mt-2">
                  {recoveryMode ? 'Reset Password' : 'Set Password & Continue'}
                </Button>
              </form>
            </>
          )}
