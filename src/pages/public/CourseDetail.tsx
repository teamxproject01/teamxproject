...
export default function CourseDetail() {
  ...
  // Scrollspy: highlight the active tab based on scroll position
  useEffect(() => {
  ...
  const heroImg = course.image_url || levelImages[course.level] || levelImages.undergraduate;
  ...
  return (
    <div className="bg-white">
      ...
      <div className="relative h-[420px] md:h-[520px] overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#1e3a8a]/70 via-[#1e3a8a]/25 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#1e3a8a]/35 via-transparent to-transparent" />
      ...
      <div className="sticky top-16 z-30 bg-white border-b border-slate-100 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="hidden sm:flex rounded-xl border border-slate-200 overflow-hidden flex-shrink-0">
              <button
              ...
              >
                <Home className="w-3.5 h-3.5" /> Domestic
              </button>
              <button
          ...
          {/* Mobile toggle */}
          <div className="flex sm:hidden rounded-xl border border-slate-200 overflow-hidden mt-2 mb-2">
            <button
              onClick={() => setStudentType('domestic')}
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold transition-colors ${
                studentType === 'domestic' ? 'bg-blue-700 text-white' : 'bg-white text-slate-500'
              }`}
            >
              <Home className="w-3.5 h-3.5" /> Domestic
            </button>
            <button
              onClick={() => setStudentType('international')}
              className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold transition-colors ${
                studentType === 'international' ? 'bg-blue-700 text-white' : 'bg-white text-slate-500'
              }`}
            >
              <Globe className="w-3.5 h-3.5" /> International
            </button>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid md:grid-cols-3 gap-10">
          ...
          <div className="md:col-span-2">
            ...
            <section id="entry" ref={(el) => { sectionRefs.current['entry'] = el; }} className="scroll-mt-32 mb-16">
              <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} key="entry">
                ...
                <motion.div variants={fadeUp} className="prose prose-slate max-w-none">
                  <p className="text-slate-600 leading-relaxed mb-6">
                    {(studentType === 'domestic'
                      ? course.entry_requirements_domestic
                      : course.entry_requirements_international)
                      ?? course.entry_requirements
                      ?? 'Entry requirements for this program ensure students have the foundational knowledge and skills to succeed in their studies.'}
                  </p>
                  {[
                    ...
                    <motion.div key={heading} variants={fadeUp} className="mb-8">
                      <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                        <CheckCircle className="w-5 h-5 text-blue-700" /> {heading}
                      </h3>
            ...
            <section id="structure" ref={(el) => { sectionRefs.current['structure'] = el; }} className="scroll-mt-32 mb-16">
              <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} key="structure">
                ...
                {course.intake_dates && (
                  <motion.div variants={fadeUp} className="mt-8 p-6 bg-blue-50 rounded-2xl border border-blue-100">
                    <h3 className="font-bold text-slate-900 mb-3 flex items-center gap-2">
                      <Calendar className="w-5 h-5 text-blue-700" /> Intake Dates
                    </h3>
            ...
            <section id="fees" ref={(el) => { sectionRefs.current['fees'] = el; }} className="scroll-mt-32 mb-16">
              <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} key="fees">
                ...
                {course.fees && (
                  <motion.div variants={fadeUp} className="bg-white border border-slate-200 rounded-2xl overflow-hidden mb-8">
                    <div className="bg-[#1e3a8a] px-6 py-4">
                      ...
                      <p className="text-blue-300 text-sm mt-1">Per year of study</p>
                    </div>
                    <div className="divide-y divide-slate-100">
                      <div className={`flex justify-between items-center px-6 py-5 transition-colors ${studentType === 'domestic' ? 'bg-blue-50/60' : ''}`}>
                        <div>
                        ...
                        <div className={`font-extrabold ${studentType === 'domestic' ? 'text-2xl text-blue-700' : 'text-lg text-slate-400'}`}>${course.fees.domestic?.toLocaleString()}</div>
                      </div>
                      <div className={`flex justify-between items-center px-6 py-5 transition-colors ${studentType === 'international' ? 'bg-blue-50/60' : ''}`}>
                        <div>
                        ...
                        <div className={`font-extrabold ${studentType === 'international' ? 'text-2xl text-blue-700' : 'text-lg text-slate-400'}`}>${course.fees.international?.toLocaleString()}</div>
                      </div>
                    </div>
                  </motion.div>
                )}
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      <div key={name} className="p-5 bg-gradient-to-r from-blue-50 to-white border border-blue-100 rounded-2xl flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <div className="flex-shrink-0 text-lg font-extrabold text-blue-700 bg-blue-100 px-4 py-2 rounded-xl whitespace-nowrap">{value}</div>
                      </div>
                    ))}
                  </div>
                </motion.div>
            ...
            <section id="apply" ref={(el) => { sectionRefs.current['apply'] = el; }} className="scroll-mt-32">
              <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} key="apply">
                ...
                <motion.div variants={fadeUp} className="mt-6 flex flex-col sm:flex-row gap-3">
                  {profile ? (
                    <Link to="/student/new-application" className="btn-primary flex-1 justify-center text-sm">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-[#1e3a8a] rounded-2xl p-6 text-white">
              ...
              <p className="text-blue-200 text-sm mb-5">Applications open now for Feb & July 2025 intakes.</p>
              {profile ? (
                <Link to="/student/new-application" className="flex items-center justify-center gap-2 w-full bg-white text-[#1e3a8a] font-bold py-3 rounded-xl hover:bg-blue-50 transition-colors text-sm">
                  Apply Now <ArrowRight className="w-4 h-4" />
                </Link>
              ) : (
                <Link to="/signup" className="flex items-center justify-center gap-2 w-full bg-white text-[#1e3a8a] font-bold py-3 rounded-xl hover:bg-blue-50 transition-colors text-sm">
                  Apply Now <ArrowRight className="w-4 h-4" />
                </Link>
              )}
            ...
            <div className="bg-white border-2 border-blue-100 rounded-2xl p-6">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-blue-700 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition-colors text-sm"
              >
          ...
          >
            ...
            <form className="space-y-4" onSubmit={async (e) => {
              ...
              const bookingData = {
                ...
                subject: '1:1 Session Booking',
                message: `Meeting type: ${formData.get('meeting_type')}\nMessage: ${formData.get('message') || 'N/A'}`,
              };
              await supabase.from('contact_enquiries').insert(bookingData);
              setBookingOpen(false);
              alert('Session request submitted! We\'ll contact you within 24 hours.');
            }}>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Full Name</label>
                <input type="text" name="name" required placeholder="Your full name" className="input-field" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="tel" name="phone" placeholder="+61 4XX XXX XXX" className="input-field" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Preferred Meeting Type</label>
                <select name="meeting_type" className="input-field">
              ...
              <div>
                ...
                <textarea rows={3} name="message" placeholder="Any specific questions or topics you'd like to discuss…" className="input-field resize-none" />
              </div>
              <div className="flex gap-3 pt-2">
