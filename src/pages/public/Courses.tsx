...
import { Search, Filter, Clock, MapPin, Award, ArrowRight, ChevronDown, ChevronUp, BookOpen, Globe, Home } from 'lucide-react';
    ...
    <div>
      ...
      <section className="relative pt-32 pb-20 bg-hero-gradient overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <p className="text-blue-700 font-semibold text-sm uppercase tracking-widest mb-3">{t('courses_tag', lang)}</p>
          ...
          {loading ? (
            ...
            >
              {filtered.map((course) => {
                ...
                const coverImg = course.image_url || courseImages[course.level] || courseImages.undergraduate;
                return (
                  <motion.div
                    key={course.id}
                    variants={fadeUp}
                      ...
                      <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-500">
                        ...
                        {course.fees && (
                          <span className="flex items-center gap-1.5 font-semibold text-blue-700">
                            {studentType === 'domestic' ? <Home className="w-4 h-4" /> : <Globe className="w-4 h-4" />}
                            ${studentType === 'domestic' ? course.fees.domestic?.toLocaleString() : course.fees.international?.toLocaleString()}/yr
                          </span>
                        )}
                      </div>
                    </div>
                            ...
                            {course.fees && (
                              <div className="bg-white rounded-xl p-5 border border-slate-200">
                                <div className="flex items-center justify-between mb-3">
                                  <h4 className="font-bold text-slate-900">{t('courses_fees', lang)}</h4>
                                  <div className="flex rounded-lg border border-slate-200 overflow-hidden text-xs">
                                    <button
                                      onClick={(e) => { e.stopPropagation(); setStudentType('domestic'); }}
                                      className={`px-2.5 py-1 font-semibold transition-colors ${studentType === 'domestic' ? 'bg-blue-700 text-white' : 'bg-white text-slate-500'}`}
                                    >Domestic</button>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); setStudentType('international'); }}
                                      className={`px-2.5 py-1 font-semibold transition-colors ${studentType === 'international' ? 'bg-blue-700 text-white' : 'bg-white text-slate-500'}`}
                                    >Intl</button>
                                  </div>
                                </div>
                                <div className="space-y-2 text-sm">
                                  <div className="flex justify-between">
                                    ...
                                    <span className={`font-bold ${studentType === 'domestic' ? 'text-blue-700 text-lg' : 'text-slate-900'}`}>${course.fees.domestic?.toLocaleString()}</span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-slate-500">{t('courses_intl', lang)}</span>
                                    <span className={`font-bold ${studentType === 'international' ? 'text-blue-700 text-lg' : 'text-slate-900'}`}>${course.fees.international?.toLocaleString()}</span>
                                  </div>
                                </div>
                              </div>
                            )}
