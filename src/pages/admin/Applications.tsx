...
const STATUS_LABELS: Record<string, string> = {
  ...
  waiting_for_payment: 'Awaiting Payment', paid_coe_issued: 'Paid / COE Issued',
  ...
  const addApproval = async () => {
    ...
    if (selected.student_id) {
      await supabase.from('messages').insert({
        ...
        content: `Your application for ${selected.courses?.title ?? 'your course'} has been approved by ${approvalPrintName.trim()}.${approvalComment ? `\n\nNote: ${approvalComment}` : ''}`,
      });
    }
    ...
    try {
      ...
      const courseTitle = selected.courses?.title ?? 'your course';
      const adminNote = approvalComment ? `\n\nMessage from MIHE: ${approvalComment}` : '';

      // Email the student
      if (selected.student_email) {
        const studentSubject = 'Your MIHE Application Has Been Approved';
        const studentHtml = `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #1e3a5f;">MIHE Admissions</h2>
          <p>Hi ${selected.student_full_name || 'there'},</p>
          <p>Great news! Your application for <strong>${courseTitle}</strong> has been <strong style="color: #16a34a;">approved</strong>.</p>
          ${approvalComment ? `<div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin: 16px 0;"><p style="color: #64748b; font-size: 13px; margin-bottom: 8px;">Message from MIHE:</p><p>${approvalComment}</p></div>` : ''}
        ...
        await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-email`, {
          ...
          body: JSON.stringify({ to: selected.student_email, subject: studentSubject, html: studentHtml }),
        });
      }

      // Email the agent if application came from an agent
      if (selected.agent_id) {
        const { data: agentProfile } = await supabase.from('profiles').select('email').eq('id', selected.agent_id).maybeSingle();
        const agentEmail = agentProfile?.email;
        if (agentEmail) {
          const agentSubject = `Student Application Approved — ${selected.student_full_name}`;
          const agentHtml = `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #1e3a5f;">MIHE Admissions</h2>
            <p>Hi,</p>
            <p>The application for your student <strong>${selected.student_full_name}</strong> (${courseTitle}) has been <strong style="color: #16a34a;">approved</strong> by ${approvalPrintName.trim()}.</p>
            ${approvalComment ? `<div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin: 16px 0;"><p style="color: #64748b; font-size: 13px; margin-bottom: 8px;">Message from MIHE:</p><p>${approvalComment}</p></div>` : ''}
      ...
      // Email send failure should not block the approval
    }
