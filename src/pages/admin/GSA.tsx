...
function EditYesNo({ value, onChange }: { value: boolean | null; onChange: (v: boolean) => void }) {
  return (
...
export default function GSAAdmin() {
  ...
  const startEdit = () => {
    if (!selected) return;
    setEditData({
      gsa_immigration_history_has: selected.gsa_immigration_history_has ?? null,
      gsa_immigration_history_details: selected.gsa_immigration_history_details ?? '',
      gsa_choice_reason: selected.gsa_choice_reason ?? '',
      gsa_has_previous_coe: selected.gsa_has_previous_coe ?? null,
      gsa_previous_coes: selected.gsa_previous_coes ?? [],
      gsa_studied_in_australia: selected.gsa_studied_in_australia ?? null,
      gsa_previous_australia_study: selected.gsa_previous_australia_study ?? [],
      gsa_has_study_gaps: selected.gsa_has_study_gaps ?? null,
      gsa_study_gaps: selected.gsa_study_gaps ?? [],
      gsa_current_circumstances: selected.gsa_current_circumstances ?? '',
      gsa_funding_source: selected.gsa_funding_source ?? '',
      gsa_estimated_tuition: selected.gsa_estimated_tuition ?? '',
      gsa_estimated_living: selected.gsa_estimated_living ?? '',
      gsa_financial_details: selected.gsa_financial_details ?? '',
    });
    setEditMode(true);
  };
  ...
  const saveEdit = async () => {
    if (!selected) return;
    setSavingEdit(true);
    const updatePayload: Record<string, any> = { ...editData, updated_at: new Date().toISOString() };

    if (selected.gsa_status === 'approved' || selected.gsa_status === 'rejected') {
      updatePayload.gsa_status = 'reviewed';
    }

    await supabase.from('agent_applications').update(updatePayload).eq('id', selected.id);

    await supabase.from('audit_logs').insert({
      user_id: user?.id,
      action: 'gsa_answers_edited',
      target_table: 'agent_applications',
      target_id: selected.id,
      metadata: {
        student_name: selected.student_full_name,
        previous_status: selected.gsa_status,
        new_status: updatePayload.gsa_status ?? selected.gsa_status,
      },
    });
  ...
  const updateArr = (key: string, i: number, patch: Record<string, any>) => {
    const arr = [...(editData[key] ?? [])];
    arr[i] = { ...arr[i], ...patch };
    setEditData({ ...editData, [key]: arr });
  };
  const removeArrItem = (key: string, i: number) => {
    setEditData({ ...editData, [key]: (editData[key] ?? []).filter((_: any, idx: number) => idx !== i) });
  };
  const addArrItem = (key: string, blank: Record<string, any>) => {
    setEditData({ ...editData, [key]: [...(editData[key] ?? []), blank] });
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="card p-6 lg:sticky lg:top-4 max-h-[calc(100vh-2rem)] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={startEdit}
                    className="flex items-center gap-1 text-xs font-semibold text-blue-700 hover:text-blue-800 px-2 py-1 rounded-lg hover:bg-blue-50 transition-colors"
                  >
      ...
      {/* Edit Modal */}
      {editMode && selected && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 overflow-y-auto" onClick={() => setEditMode(false)}>
          <div className="card p-6 w-full max-w-2xl my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-slate-900 flex items-center gap-2">
                <Pencil className="w-4 h-4 text-blue-700" /> Edit GSA Answers
              </h3>
            ...
            <p className="text-xs text-slate-500 mb-4">
              Student: <span className="font-semibold text-slate-700">{selected.student_full_name}</span> • Current status: <span className={`badge text-xs ${GSA_STATUS_COLORS[selected.gsa_status ?? 'not_started']}`}>{selected.gsa_status}</span>
            </p>

            {(selected.gsa_status === 'approved' || selected.gsa_status === 'rejected') && (
              <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-100 rounded-lg mb-4">
                <AlertCircle className="w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5" />
                <p className="text-xs text-amber-700">
                  This GSA has already been <strong>{selected.gsa_status}</strong>. Saving edits will reset the status to <strong>reviewed</strong> so it can be re-assessed.
                </p>
              </div>
            )}
            ...
            <div className="space-y-5 max-h-[calc(100vh-280px)] overflow-y-auto pr-2">
              {/* Immigration History */}
              <div>
                <label className="label">Prior visa refusal/cancellation/deportation?</label>
                <EditYesNo value={editData.gsa_immigration_history_has} onChange={(v) => setEditData({ ...editData, gsa_immigration_history_has: v })} />
                {editData.gsa_immigration_history_has === true && (
                  <textarea
                    value={editData.gsa_immigration_history_details ?? ''}
                    onChange={(e) => setEditData({ ...editData, gsa_immigration_history_details: e.target.value })}
              ...
              {/* Choice of Course */}
              <div>
                <label className="label">Reason for course choice</label>
                <textarea
                  value={editData.gsa_choice_reason ?? ''}
                  onChange={(e) => setEditData({ ...editData, gsa_choice_reason: e.target.value })}
              ...
              {/* Previous CoE */}
              <div>
                <label className="label">Held previous CoE?</label>
                <EditYesNo value={editData.gsa_has_previous_coe} onChange={(v) => setEditData({ ...editData, gsa_has_previous_coe: v })} />
                {editData.gsa_has_previous_coe === true && (
                  <div className="space-y-2 mt-2">
                    {(editData.gsa_previous_coes ?? []).map((coe: any, i: number) => (
                      <div key={i} className="p-3 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <button type="button" onClick={() => removeArrItem('gsa_previous_coes', i)} className="text-slate-400 hover:text-red-500">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                        <input type="text" value={coe.institution ?? ''} onChange={(e) => updateArr('gsa_previous_coes', i, { institution: e.target.value })} className="input-field" placeholder="Institution" />
                        <input type="text" value={coe.course ?? ''} onChange={(e) => updateArr('gsa_previous_coes', i, { course: e.target.value })} className="input-field" placeholder="Course" />
                        <div className="grid grid-cols-2 gap-2">
                          <input type="date" value={coe.start_date ?? ''} onChange={(e) => updateArr('gsa_previous_coes', i, { start_date: e.target.value })} className="input-field" />
                          <input type="date" value={coe.end_date ?? ''} onChange={(e) => updateArr('gsa_previous_coes', i, { end_date: e.target.value })} className="input-field" />
                        </div>
                        <textarea value={coe.reason_for_withdrawing ?? ''} onChange={(e) => updateArr('gsa_previous_coes', i, { reason_for_withdrawing: e.target.value })} className="input-field min-h-[60px] resize-y" placeholder="Reason for withdrawing" />
                      </div>
                    ))}
                    <button type="button" onClick={() => addArrItem('gsa_previous_coes', { institution: '', course: '', start_date: '', end_date: '', reason_for_withdrawing: '' })} className="flex items-center gap-2 text-sm font-semibold text-blue-700 hover:text-blue-800">
              ...
              <div>
                <label className="label">Previously studied in Australia?</label>
                <EditYesNo value={editData.gsa_studied_in_australia} onChange={(v) => setEditData({ ...editData, gsa_studied_in_australia: v })} />
                {editData.gsa_studied_in_australia === true && (
                  <div className="space-y-2 mt-2">
                    {(editData.gsa_previous_australia_study ?? []).map((s: any, i: number) => (
                      <div key={i} className="p-3 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-semibold text-slate-500">Entry {i + 1}</span>
                          <button type="button" onClick={() => removeArrItem('gsa_previous_australia_study', i)} className="text-slate-400 hover:text-red-500">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                        <input type="text" value={s.institution ?? ''} onChange={(e) => updateArr('gsa_previous_australia_study', i, { institution: e.target.value })} className="input-field" placeholder="Institution" />
                        <input type="text" value={s.course ?? ''} onChange={(e) => updateArr('gsa_previous_australia_study', i, { course: e.target.value })} className="input-field" placeholder="Course" />
                        <div className="grid grid-cols-2 gap-2">
                          <input type="date" value={s.start_date ?? ''} onChange={(e) => updateArr('gsa_previous_australia_study', i, { start_date: e.target.value })} className="input-field" />
                          <input type="date" value={s.end_date ?? ''} onChange={(e) => updateArr('gsa_previous_australia_study', i, { end_date: e.target.value })} className="input-field" />
                        </div>
                      </div>
                    ))}
                    <button type="button" onClick={() => addArrItem('gsa_previous_australia_study', { institution: '', course: '', start_date: '', end_date: '' })} className="flex items-center gap-2 text-sm font-semibold text-blue-700 hover:text-blue-800">
                      <Plus className="w-4 h-4" /> Add entry
                    </button>
                  </div>
                )}
              ...
              {/* Study Gaps */}
              <div>
                <label className="label">Has study gaps?</label>
                <EditYesNo value={editData.gsa_has_study_gaps} onChange={(v) => setEditData({ ...editData, gsa_has_study_gaps: v })} />
                {editData.gsa_has_study_gaps === true && (
                  <div className="space-y-2 mt-2">
                    {(editData.gsa_study_gaps ?? []).map((g: any, i: number) => (
                      <div key={i} className="p-3 rounded-xl bg-slate-50 border border-slate-100 space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-semibold text-slate-500">Entry {i + 1}</span>
                          <button type="button" onClick={() => removeArrItem('gsa_study_gaps', i)} className="text-slate-400 hover:text-red-500">
                            <Trash2 className="w-4 h-4" />
                          </button>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <input type="date" value={g.start_date ?? ''} onChange={(e) => updateArr('gsa_study_gaps', i, { start_date: e.target.value })} className="input-field" />
                          <input type="date" value={g.end_date ?? ''} onChange={(e) => updateArr('gsa_study_gaps', i, { end_date: e.target.value })} className="input-field" />
                        </div>
                        <textarea value={g.details_of_gap ?? ''} onChange={(e) => updateArr('gsa_study_gaps', i, { details_of_gap: e.target.value })} className="input-field min-h-[60px] resize-y" placeholder="Details of gap" />
                      </div>
                    ))}
                    <button type="button" onClick={() => addArrItem('gsa_study_gaps', { start_date: '', end_date: '', details_of_gap: '' })} className="flex items-center gap-2 text-sm font-semibold text-blue-700 hover:text-blue-800">
                      <Plus className="w-4 h-4" /> Add entry
                    </button>
                  </div>
                )}
              ...
              <div>
                <label className="label">Current circumstances</label>
                <textarea
                  value={editData.gsa_current_circumstances ?? ''}
                  onChange={(e) => setEditData({ ...editData, gsa_current_circumstances: e.target.value })}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" value={editData.gsa_funding_source ?? ''} onChange={(e) => setEditData({ ...editData, gsa_funding_source: e.target.value })} className="input-field" />
                </div>
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    <label className="label">Estimated Tuition (AUD)</label>
                    <input type="text" value={editData.gsa_estimated_tuition ?? ''} onChange={(e) => setEditData({ ...editData, gsa_estimated_tuition: e.target.value })} className="input-field" />
                  </div>
                  <div>
                    <label className="label">Estimated Living (AUD)</label>
                    <input type="text" value={editData.gsa_estimated_living ?? ''} onChange={(e) => setEditData({ ...editData, gsa_estimated_living: e.target.value })} className="input-field" />
                  </div>
                </div>
                <div>
                  <label className="label">Additional Financial Details</label>
                  <textarea
                    value={editData.gsa_financial_details ?? ''}
                    onChange={(e) => setEditData({ ...editData, gsa_financial_details: e.target.value })}
            ...
            <div className="flex gap-2 pt-4 border-t border-slate-100 mt-4">
              <Button onClick={saveEdit} loading={savingEdit} className="flex-1 bg-blue-700 hover:bg-blue-800">
