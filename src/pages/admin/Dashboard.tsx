...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      supabase.from('agent_applications').select('status, source', { count: 'exact' }).neq('status', 'draft'),
      supabase.from('forms').select('status', { count: 'exact' }),
      supabase.from('profiles').select('id', { count: 'exact' }).eq('role', 'student'),
      supabase.from('agent_applications').select('*, courses(title)').neq('status', 'draft').order('created_at', { ascending: false }).limit(6),
      supabase.from('application_documents').select('id', { count: 'exact' }).eq('status', 'uploaded'),
      supabase.from('agents').select('approval_status', { count: 'exact' }),
    ]).then(([apps, forms, students, recent, docs, agentsData]) => {
      ...
      setStats({
        ...
        agents: {
          ...
          pending: agentList.filter(a => a.approval_status === 'pending').length,
        },
      });
  ...
  const pieData = [
    ...
    { name: 'Other', value: Math.max(0, stats.applications.total - stats.applications.new - stats.applications.pending - stats.applications.approved - stats.applications.rejected) },
  ].filter(d => d.value > 0);

  const statCards = [
    { label: 'Total Students', value: stats.students, icon: Users, color: 'bg-blue-700', trend: '+12%' },
    { label: 'New Applications', value: stats.applications.new, icon: FileText, color: 'bg-sky-500', trend: '+8%' },
    { label: 'Pending Agents', value: stats.agents.pending, icon: Users, color: 'bg-amber-500', trend: '+5%' },
