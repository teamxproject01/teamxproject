...
export default function AgentApplications() {
  ...
  const load = () => {
    Promise.all([
      supabase.from('agent_applications').select('*, courses(title, level, campus), agents(agency_name)').neq('status', 'draft').order('created_at', { ascending: false }),
  ...
  const updateStatus = async (app: AgentApplication, status: AgentApplicationStatus) => {
    ...
    await supabase.from('agent_applications').update({
      status,
      public_comment: comment || app.public_comment,
    ...
    if (appSource === 'student') {
      ...
      await supabase.from('agent_notifications').insert({
        ...
        type: ['offer_letter_sent', 'offer_accepted', 'waiting_for_signature', 'signed_offer_review'].includes(status) ? 'offer' : status === 'enrolled' || status === 'paid_coe_issued' ? 'enrolment' : status === 'documents_required' ? 'document_request' : 'status_change',
      });
    }
      ...
      if (studentEmail) {
        try {
          ...
          const subject = status === 'approved'
            ? 'Your MIHE Application Has Been Approved'
            : 'Your MIHE Application Has Been Rejected';
          const html = status === 'approved'
            ? `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #1e3a5f;">MIHE Admissions</h2>
                <p>Hi there,</p>
                <p>Great news! Your application (${app.application_id}) for <strong>${app.courses?.title ?? 'your course'}</strong> has been <strong style="color: #16a34a;">approved</strong>.</p>
                ${comment ? `<div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin: 16px 0;"><p style="color: #64748b; font-size: 13px; margin-bottom: 8px;">Message from MIHE:</p><p>${comment}</p></div>` : ''}
                <p>We will be in touch with next steps shortly.</p>
                <br><p>Best regards,<br>MIHE Admissions Team</p>
              </div>`
            : `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #1e3a5f;">MIHE Admissions</h2>
                <p>Hi there,</p>
                <p>We're sorry to inform you that your application (${app.application_id}) for <strong>${app.courses?.title ?? 'your course'}</strong> has been <strong style="color: #dc2626;">rejected</strong>.</p>
                ${comment ? `<div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin: 16px 0;"><p style="color: #64748b; font-size: 13px; margin-bottom: 8px;">Message from MIHE:</p><p>${comment}</p></div>` : ''}
                <p>If you believe this is an error or would like more information, please contact the MIHE admissions team.</p>
                <br><p>Best regards,<br>MIHE Admissions Team</p>
              </div>`;
          ...
          // Email the agent if application came from an agent
          if (appSource === 'agent' && app.agent_id && status === 'approved') {
            const { data: agentProfile } = await supabase.from('profiles').select('email').eq('id', app.agent_id).maybeSingle();
            const agentEmail = agentProfile?.email;
            if (agentEmail) {
              const agentSubject = `Student Application Approved — ${app.student_full_name}`;
              const agentHtml = `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
                <h2 style="color: #1e3a5f;">MIHE Admissions</h2>
                <p>Hi,</p>
                <p>The application for your student <strong>${app.student_full_name}</strong> (${app.courses?.title ?? 'course'}) has been <strong style="color: #16a34a;">approved</strong>.</p>
                ${comment ? `<div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin: 16px 0;"><p style="color: #64748b; font-size: 13px; margin-bottom: 8px;">Message from MIHE:</p><p>${comment}</p></div>` : ''}
                <p>Please inform the student and watch for further updates on next steps.</p>
                <br><p>Best regards,<br>MIHE Admissions Team</p>
              </div>`;
              await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-email`, {
                method: 'POST',
                ...
                body: JSON.stringify({ to: agentEmail, subject: agentSubject, html: agentHtml }),
              });
            }
          }
        } catch {
          // Email send failure should not block the status update
        }
      }
    }

    load();
    setSelected({ ...app, status, public_comment: comment || app.public_comment });
    setComment('');
    setUpdating(false);
  };
        ...
        <div className="lg:col-span-2">
          {selected ? (
            <div className="card p-6 lg:sticky lg:top-4">
              ...
              <div className="w-14 h-14 bg-blue-700 rounded-2xl flex items-center justify-center text-white text-2xl font-bold mb-4">
              ...
              <div className="space-y-3 text-sm mb-5">
                ...
                {selected.admin_comment && <div className="p-3 bg-amber-50 rounded-xl"><p className="text-xs text-amber-600">Admin Comment (Private)</p><p className="text-sm text-slate-700">{selected.admin_comment}</p></div>}
                {selected.public_comment && <div className="p-3 bg-blue-50 rounded-xl"><p className="text-xs text-blue-600">Public Comment</p><p className="text-sm text-slate-700">{selected.public_comment}</p></div>}
              </div>

              <div className="mb-5">
                <GSASummary app={selected} />
              </div>

              {!['enrolled', 'cancelled', 'rejected'].includes(selected.status) && (
                <div className="space-y-3 pt-4 border-t border-slate-100">
                  <div>
                    <label className="label flex items-center gap-1.5"><MessageSquare className="w-3.5 h-3.5" /> Public Comment (visible to applicant)</label>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    <Button size="sm" className="bg-green-600 hover:bg-green-700" loading={updating} onClick={() => updateStatus(selected, 'approved')}>
                      <CheckCircle className="w-4 h-4" /> Approved
                    </Button>
                    <Button size="sm" variant="secondary" className="bg-indigo-500 hover:bg-indigo-600 text-white border-0" loading={updating} onClick={() => updateStatus(selected, 'offer_letter_sent')}>
                      <CheckCircle className="w-4 h-4" /> Offer Sent
                    </Button>
                    <Button size="sm" variant="secondary" className="bg-amber-500 hover:bg-amber-600 text-white border-0" loading={updating} onClick={() => updateStatus(selected, 'waiting_for_signature')}>
                      <AlertCircle className="w-4 h-4" /> Wait Signature
                    </Button>
                    <Button size="sm" variant="secondary" className="bg-teal-500 hover:bg-teal-600 text-white border-0" loading={updating} onClick={() => updateStatus(selected, 'offer_accepted')}>
                      <CheckCircle className="w-4 h-4" /> Offer Accepted
                    </Button>
                    <Button size="sm" variant="secondary" className="bg-orange-500 hover:bg-orange-600 text-white border-0" loading={updating} onClick={() => updateStatus(selected, 'waiting_for_payment')}>
                      <CreditCard className="w-4 h-4" /> Ask for Payment
                    </Button>
                    <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" loading={updating} onClick={() => updateStatus(selected, 'paid_coe_issued')}>
                      <CheckCircle className="w-4 h-4" /> Paid/COE
                    </Button>
