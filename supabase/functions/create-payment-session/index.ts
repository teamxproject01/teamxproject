  ...
  try {
    ...
    const sessionRes = await fetch("https://api.stripe.com/v1/checkout/sessions", {
      ...
      body: new URLSearchParams({
        ...
        "line_items[0][name]": `${installment.label} - ${courseTitle}`,
        "line_items[0][description]": `Payment for ${app.student_full_name}`,
        "line_items[0][amount]": Math.round(parseFloat(installment.amount) * 100).toString(),
        "line_items[0][currency]": "aud",
